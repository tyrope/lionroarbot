...
def setup(bot):
    ...
    bot.memory['timer']['channel'] = '#tyrope' # The twitch channel you want this module to run in.
    bot.memory['timer']['ctt_default'] = 'kiRU4' # Default click to tweet ID
...
def timed_message(bot):
    ...
    if 'http://ctt.ec/' in msg:
        if 'ctt' not in bot.memory['timer']:
            bot.memory['timer']['ctt'] = bot.memory['timer']['ctt_default']
